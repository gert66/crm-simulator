...
def run_6plus3(true_p, start_level=1, max_n=27, accept_max_dlt=1, rng=None):
    ...
    Simple 6+3:
      ...
      - If 0/6 DLT: escalate.
      - If 1/6 DLT: expand by 3 (total 9); accept if total DLTs <= accept_max_dlt.
...
R_DEFAULTS = {
    ...
    # 6+3 defaults (you asked default 27)
    "max_n_63": 27,
    "accept_rule_63": 1,
}
...
# ============================================================
# UI: SETTINGS + PLAYGROUND (one page)
# ============================================================

st.title("Dose Escalation Simulator: 6+3 vs CRM (acute-only)")
st.caption("Window 1 is for inputs and playground. Open the Results page in a second window for outputs.")

# --- Essentials row ---
st.subheader("Essentials")
c1, c2, c3, c4, c5 = st.columns([1.1, 1.1, 1.1, 1.1, 0.9], gap="large")
...
with c1:
    st.number_input(
        ...
        key="target",
        help="Target DLT probability used by both designs."
    )
with c2:
    st.selectbox(
        "Start dose",
        ...
        key="start_level",
        help="0-based indexing. Default Level 1 equals 5×5 Gy."
    )
with c3:
    st.number_input(
        "Max sample size (CRM)",
...
with c4:
    st.number_input(
        ...
        key="cohort_size",
        help="Cohort size per CRM update."
    )
with c5:
    st.write("")
    st.write("")
    if st.button("Reset to R defaults", help="Resets all tunable settings. The True curve is kept as-is."):
        reset_all_defaults()
        st.rerun()
...
left, mid, right = st.columns([0.95, 1.05, 1.1], gap="large")
...
with left:
    ...
    manual_true = st.toggle(
        "Edit true curve",
        value=True,
        help="Ground truth used for simulation of Bernoulli acute DLT outcomes."
    )
...
with mid:
    ...
    st.radio(
        ...
        horizontal=True,
        key="prior_model",
        help="dfcrm-style skeleton generator."
    )
    st.slider(
        "Prior target",
        min_value=0.05, max_value=0.50, step=0.01,
        key="prior_target",
        help="Target used to calibrate skeleton."
    )
    st.slider(
        "Halfwidth (delta)",
        min_value=0.01, max_value=0.30, step=0.01,
        key="halfwidth",
        help="Skeleton spacing parameter."
    )
    st.slider(
        "Prior MTD (nu, 1-based)",
        min_value=1, max_value=5, step=1,
        key="prior_nu",
        help="The dose level that is set near the prior target."
    )
    ...
    st.slider(
        ...
        key="logistic_intcpt",
        help="Only relevant if skeleton model is logistic."
    )
...
# Compact CRM knobs + preview plot
with right:
    ...
    st.slider(
        ...
        key="sigma",
        help="Larger sigma means wider prior on theta."
    )
    st.toggle(
        "Burn-in until first DLT",
        key="burn_in",
        help="Escalate cohort-wise until first DLT is observed, then use CRM updates."
    )
    st.toggle(
...
# --- Remaining settings (no expander) ---
st.subheader("Simulation + design settings")
...
g1, g2, g3 = st.columns([1.0, 1.0, 1.0], gap="large")
with g1:
    st.toggle(
...
with g3:
    st.number_input(
        ...
        min_value=12, max_value=200, step=3,
        key="max_n_63",
        help="Total cap for 6+3 (default 27)."
    )

st.selectbox(
    "6+3 acceptance rule after expansion to 9",
    options=[1, 2],
...
with st.expander("Current settings (from code)", expanded=False):
    cfg = {k: st.session_state[k] for k in R_DEFAULTS.keys()}
    st.json(cfg)
...
if run:
    ...
    # "kans dat een patiënt een DLT krijgt" = overall DLT rate per patient
    total_pat_63 = float(np.sum(nmat_63))
    total_pat_crm = float(np.sum(nmat_crm))
    dlt_rate_63 = float(np.sum(dlt_63)) / total_pat_63 if total_pat_63 > 0 else 0.0
    dlt_rate_crm = float(np.sum(dlt_crm)) / total_pat_crm if total_pat_crm > 0 else 0.0

    payload = {
        "dose_labels": dose_labels,
        "target": float(st.session_state["target"]),
        "true_p": [float(x) for x in true_p],
        "true_mtd": int(true_mtd),
        "skeleton": [float(x) for x in skeleton],
        "p_mtd_63": [float(x) for x in p63],
        "p_mtd_crm": [float(x) for x in pcrm],
        "avg_n_63": [float(x) for x in avg_n63],
        "avg_n_crm": [float(x) for x in avg_ncrm],
        "dlt_rate_63": float(dlt_rate_63),
        "dlt_rate_crm": float(dlt_rate_crm),
        "n_sims": int(ns),
    ...
    if st.button("Go to Results"):
        if hasattr(st, "switch_page"):
            ...
            st.info("Use the left sidebar to open Results.")
