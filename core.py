...
class Defaults:
    ...
    max_n_6p3: int = 27
    max_n_crm: int = 27
    cohort_size_6p3: int = 3  # used for both 6+3 and CRM in simplesim.py
...
def _apply_defaults_overwrite_all() -> None:
    """
    Hard overwrite of all known keys to defaults.
    Called only on first init or when reset was requested.
    """
    s = st.session_state
...
def sync_widget_keys(force_defaults: bool = False) -> None:
    """
    Convenience helper used by Playground.
    If force_defaults=True, overwrite all session keys to DEFAULTS.
    Otherwise, only ensures missing keys exist + true curve widget keys exist.
    """
    if force_defaults:
        _apply_defaults_overwrite_all()
        return
...
def build_payload() -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        ...
        # sample sizes / cohorts
        "max_n_6p3": int(s.get("max_n_6p3", DEFAULTS.max_n_6p3)),
        "max_n_crm": int(s.get("max_n_crm", DEFAULTS.max_n_crm)),
