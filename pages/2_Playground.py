...
with col1:
    ...
    if st.session_state["edit_true_curve"]:
        # compact numeric inputs (no state mutation tricks)
        new_vals = []
        ...
        # commit once
        st.session_state["true_curve"] = new_vals
        true_curve = new_vals
    else:
        # read-only view
        for i, lab in enumerate(labels):
    ...
    # True MTD marker (closest to target)
...
with col3:
    ...
    st.slider("EWOC alpha", min_value=0.01, max_value=0.50, step=0.01, key="ewoc_alpha", disabled=(not st.session_state["ewoc_enable"]))
...
if res is not None:
    ...
    # Verwachte keys in res (pas aan op jouw simplesim output)
    # We proberen generiek te lezen:
    # - res["mtd_probs_6p3"], res["mtd_probs_crm"] arrays length n_dose
    # - res["avg_n_per_dose_6p3"], res["avg_n_per_dose_crm"]
    # - res["p_dlt_per_patient_6p3"], res["p_dlt_per_patient_crm"]
    # Als jouw output anders heet, hernoem alleen deze mapping.
    ...
    r1, r2, r3 = st.columns([1.25, 1.25, 0.8])
