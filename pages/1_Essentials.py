...
def _sigma_to_kappa(prior_sigma_theta: float) -> float:
    """
    Smaller sigma => stronger prior.
    Simple mapping: kappa ~ 1/sigma^2 scaled.
    """
    sigma = float(max(0.10, prior_sigma_theta))
    base = 6.0
    kappa = base / (sigma * sigma)
...
def _simulate_crm_trial(
    ...
    # incorporate already-treated-at-start with 0 DLT
    if n_prior_start_no_dlt > 0:
        add_n = int(n_prior_start_no_dlt)
        treated[dose] += add_n
        total_treated += add_n

    # prior strength
    kappa = _sigma_to_kappa(prior_sigma_theta)
    a0 = np.clip(skeleton * kappa, 0.5, None)
    b0 = np.clip((1.0 - skeleton) * kappa, 0.5, None)

    # if "already treated" already exhausts max_n, just return posterior-mean MTD
    ...
    while total_treated < max_n:
        ...
        if ewoc_enable:
            ...
            if not allowed[cand]:
                safe = np.where(allowed)[0]
                cand = int(safe[np.argmin(np.abs(p_mean[safe] - target))]) if len(safe) else int(np.argmin(p_mean))
...
def run_simulations(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    max_n_crm = int(payload["max_n_crm"])
    ...
    burnin_until_first_dlt = bool(payload.get("burnin_until_first_dlt", True))
