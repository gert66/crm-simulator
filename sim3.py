...
def overdose_ok(alpha_grid, post_w, skeleton, dose, theta_over, cutoff):
    """
    EWOC-style rule:
    ok if P(p_d > theta_over) <= cutoff
    """
    pd = crm_p(skeleton[dose], alpha_grid)
    prob_over = float(np.sum(post_w * (pd > theta_over)))
    return prob_over <= cutoff, prob_over

def enforce_overdose_rule(alpha_grid, post_w, skeleton, rec, theta_over, od_cutoff):
    """
    Step down until safe, but do not go below 0.
    Returns safe dose index.
    """
    cand = int(rec)
    while cand > 0:
        ok, _ = overdose_ok(alpha_grid, post_w, skeleton, cand, theta_over, od_cutoff)
        if ok:
            break
        cand -= 1
    return cand
...
def simulate_one_trial(true_p, skeleton, N, target,
                       ...
                       overdose_control=False, theta_over=0.20, od_cutoff=0.25,
    ...
    for _ in range(int(N)):
        ...
        # Optional early stop: if even dose 1 is unsafe under the EWOC rule
        if overdose_control and stop_if_dose1_unsafe:
            ok1, _ = overdose_ok(alpha_grid, post_w, skeleton, 0, theta_over, od_cutoff)
            if not ok1:
                break
        ...
        if overdose_control:
            current = enforce_overdose_rule(alpha_grid, post_w, skeleton, rec, theta_over, od_cutoff)
        else:
            current = int(rec)

    # Final recommendation, also filtered through overdose control (this was missing before)
    rec_final, _ = recommend_dose(alpha_grid, post_w, skeleton, target)
    if overdose_control:
        rec_final = enforce_overdose_rule(alpha_grid, post_w, skeleton, rec_final, theta_over, od_cutoff)

    return int(rec_final), np.array(d), np.array(y)
...
def run_sims(n_sims, true_p, skeleton, N, target,
    ...
    finals = np.zeros(int(n_sims), dtype=int)
    dlts = np.zeros(int(n_sims), dtype=int)
    alloc = np.zeros((int(n_sims), K), dtype=int)
    ...
    res = {
        ...
        "final_recommendation_dist": (np.bincount(finals, minlength=K) / int(n_sims)).tolist(),
...
with col1:
    ...
    overdose_control = st.checkbox("Use overdose control (EWOC-style)", value=True)

    theta_over = st.number_input(
        "Overdose threshold (toxicity level considered unsafe)",
        min_value=0.01, max_value=0.99, value=0.20, step=0.01
    )

    od_cutoff = st.number_input(
        "Overdose cutoff: P(p_d > overdose threshold) must be ≤",
        min_value=0.01, max_value=0.99, value=0.25, step=0.05
    )

    stop_if_dose1_unsafe = st.checkbox(
        "Stop trial if even dose level 1 appears unsafe",
        value=True
    )
